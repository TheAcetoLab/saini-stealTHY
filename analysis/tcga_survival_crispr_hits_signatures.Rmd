...
chunk_data %<>%
  bind_rows(cox_meta_add) %>%
  mutate(
    # project_id = factor(project_id, levels= c('Meta', '', gsub("TCGA-", "", rev(tcga_selected)))),
    project_id = factor(project_id, levels= c('Meta', '', rev(tcga_selected))),
    project_type = ifelse(project_id == 'Meta', 'Meta', 'Project'),
    FDR = p.adjust(p.value, method = 'BH'),
    logP = -log10(p.value),
...
```{r signature-maxstat-coxph-ggplot-forestplot-dss-dfi-vertical, fig.width = 2.8, fig.asp = 0.6}
...
## Figure 6E: Kaplan-Meier plots  - Optimal cut-off
```{r signature-km-conf}
use_survfit_km <- survfit_km_maxstat_list
```


```{r signature-km, fig.width = 3.2, fig.asp = 1.2}
use_lwd <- (1/2.141959)/4
