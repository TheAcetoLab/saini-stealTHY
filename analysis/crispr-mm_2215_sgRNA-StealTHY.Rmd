...
### Figure 5B: sgRNA distribution in mice transplanted with 4T1 cells previously subjected to StealTHY KO
...
### Supplementary figure 5B: sgRNA distribution in mice transplanted with 4T1 cells previously subjected to StealTHY KO
...
### Supplementary Figure 5D left: Dip test table for Syngeneic and NSG models
...
### Figure 5C and Supplementary Figure 5F
...
### Figure 5D: Fold-change of non-expressed hits
```{r non-expressed-hits-barplot-lfc, fig.width=1.5, fig.asp=1}
chunk_data <- hits_stats_global
max_lfc <- max(chunk_data$lfc[chunk_data$lfc < Inf], na.rm = TRUE)
max_lfc <- ceiling(max_lfc + 0.5)
min_lfc <- min(chunk_data$lfc[chunk_data$lfc > -Inf], na.rm = TRUE)
min_lfc <- -1*ceiling(abs(min_lfc) + 0.5)

chunk_data %<>%
  mutate(
    lfc = ifelse(lfc == Inf, max_lfc, lfc),
    lfc = ifelse(lfc == -Inf, min_lfc, lfc),
  )

y_range <- range(chunk_data$lfc, na.rm = TRUE) %>% abs
y_range <- y_range * c(-1, 1)
y_breaks <- seq(y_range[1], y_range[2], by = 0.5)
y_labels <- as.character(y_breaks)
y_labels[y_breaks == min_lfc] <- '-\u221e'
y_labels[y_breaks == max_lfc] <- '\u221e'
...
### Figure 5D: Proportion of non-expressed hits
...
### Supplementary figure 5G: Fold-change of non-expressed hits
...
### Supplementary figure 5G: Proportion of non-expressed hits
