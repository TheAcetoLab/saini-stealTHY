...
### Figure 3C top: Differential expressed genes in dCas9-Thy1 over Thy1 transduction
...
### Figure 3C bottom: Representative gene-sets enriched in dCas9-Thy1 over Thy1 transduction
...
```{r sample-clustering-hvg-generation}
...
```{r sample-clustering-heatmap-generation}
comp <- names(clustering_groups)[1]
# for(comp in names(clustering_groups)) {
...
### Figure 4G: Sample distances in MVT1 models

Samples obtained from syngeneic hosts, following transplantation of mammary carcinoma cells transduced with integrating dCas9-Thy1 (LV Cas9) or transduced with HK and transfected with Apo-Cas9, then MACS-sorted as TrCD8a-positive and TrCD8a-negative cells

```{r sample-clustering-heatmap-mvt1, fig.width = 2, fig.asp = 1.4}
draw(clustering_heatmaps[['MVT1']],  heatmap_legend_side = "bottom")
cat("\n\n")
```


### Supplementary figure 4D left: Sample distances in Py2T model

Samples obtained from syngeneic hosts, following transplantation of mammary carcinoma cells transduced with integrating dCas9-Thy1 (LV Cas9) or transduced with HK and transfected with Apo-Cas9, then MACS-sorted as TrCD8a-positive and TrCD8a-negative cells

```{r sample-clustering-heatmap-py2t, fig.width = 2, fig.asp = 1.4}
draw(clustering_heatmaps[['Py2T']],  heatmap_legend_side = "bottom")
cat("\n\n")
```

### Supplementary figure 4D right: Sample distances in 4T1 model

Samples obtained from syngeneic hosts, following transplantation of mammary carcinoma cells transduced with integrating dCas9-Thy1 (LV Cas9) or transduced with HK and transfected with Apo-Cas9, then MACS-sorted as TrCD8a-positive and TrCD8a-negative cells

```{r sample-clustering-heatmap-4t1, fig.width = 2, fig.asp = 1.4}
...
```{r qrt-pcr-heatmap-trcd8pos, fig.width = 0.90, fig.asp = 1.4}
mat <- matrix(
...
```{r qrt-pcr-heatmap-trcd8neg, fig.width = 0.90, fig.asp = 1.4}
mat <- matrix(
...
### Figure 4D : qRT-PCR in TrCD8a+ and TrCD8a-
```{r qrt-pcr-heatmap-combined2, fig.width = 1.80, fig.asp = 0.80}
