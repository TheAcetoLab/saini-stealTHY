...
```{r complexity-dynamics-padj-panel}
chunk_data %>% 
  ggplot(aes(vector_short, guides_over_30_pct,
             color = vector_short,
             fill = vector_short)) +
...
```{r complexity-dynamics-padj-group}
chunk_data %>% 
  ggplot(aes(vector_short, guides_over_30_pct,
             color = vector_short,
             fill = vector_short)) +
...
x <- colData(se) %>% data.frame %>% 
  mutate(
    vector_mhost_selected = ifelse(vector_mhost == 'Thy1, Syngeneic', '')
  )
...
y=x %>% 
    filter(
      ...
      vector_mhost_selected = ifelse()
      vector_mmodel %in% c('4T1-KO-Thy1-BALB', 'MVT1-KO-Thy1-FVB', 'Py2T-KO-Thy1-FVB'))

# chunk_data <- colData(se) %>% data.frame %>%
#   filter(sample_type %in% c('cell_culture', 'Primary tumor')) %>% 
#   filter(!is.na(vector_short)) %>% 
#   filter(effect_type_short == 'noKO') %>% 
#   mutate(
#     guides_over_30_pct = guides_over_30 / 2215
