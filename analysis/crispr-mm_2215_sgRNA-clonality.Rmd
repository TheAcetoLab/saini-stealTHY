...
### Figure 4H: Second draft
```{r complexity-dynamics-4h-v2-conf}
se <- readRDS(file.path(params$data_dir, 'se.rds'))
...
"+Apo-Cas9","ColoCT26_StealTHY_Tum_1","In vivo (colon cecal wall)"
)
...
#### Treshold 30x
```{r complexity-dynamics-4h-v2-30x, fig.asp = 4, fig.width = 1.2}
chunk_data %>% 
  ggplot(aes(group, guides_over_30_pct,
             color = group,
             fill = group)) +
...
#### Treshold 100x
```{r complexity-dynamics-4h-v2-100x, fig.asp = 1.8, fig.width = 1.5}
chunk_data %>% 
  ggplot(aes(group, guides_over_100_pct,
             color = group,
             fill = group)) +
...
##### Main figure
```{r complexity-dynamics-3b-30x, fig.width = 2, fig.asp = 1}
chunk_data %>% 
...
```{r complexity-dynamics-3b-30x-pval-y, fig.width = 1.5, fig.asp = 1.5}
chunk_data %>% 
...
##### Main figure
```{r complexity-dynamics-3b-100x, fig.width = 2, fig.asp = 1}
chunk_data %>% 
...
##### Figure with Y-Pvalues
```{r complexity-dynamics-3b-100x-pval-y, fig.width = 1.5, fig.asp = 1.5}
chunk_data %>% 
  mutate(group = factor(as.character(group), levels = c('Ig control', 'anti-CD4/CD8', 'untreated'))) %>% 
  ggplot(aes(group, guides_over_100_pct,
             color = group,
...
```{r complexity-dynamics-3b-B-pval, fig.width = 1.5, fig.asp = 1.5}
chunk_data %>% 
  mutate(group = factor(as.character(group), levels = c('Ig control', 'anti-CD4/CD8', 'untreated'))) %>% 
