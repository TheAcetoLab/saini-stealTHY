...
```{r r complexity-dynamics-panel-fdr, fig.width=2.8, fig.asp=1.6}
chunk_data %>% 
  ggplot(aes(vector_short, guides_over_30_pct,
             color = vector_short,
             fill = vector_short)) +
...
```{r r complexity-dynamics-group-fdr, fig.width=2.8, fig.asp=1.6}
chunk_data %>% 
  ggplot(aes(vector_short, guides_over_30_pct,
             color = vector_short,
             fill = vector_short)) +
...
Plot usign Massimo's data
```{r complexity-dynamics2, fig.width=2.8, fig.asp=1.6}
x <- read_csv('~/Downloads/Fig\ 2D\ source\ data.csv')

x %<>% 
  mutate(
    pct_shown_in_figure = `% shown in figure`,
    sample_type= ifelse(is.na(`Lung mets?`), 'Primary tumor', 'Lung mets'),
    sample_type = factor(sample_type, levels = c('Primary tumor', 'Lung mets')),
    vector_short = ifelse(`vector payload` == "lentiGuide-Thy1", "Thy1", `vector payload`),
    vector_short = ifelse(`vector payload` == "lentiGuide-Puro", "Puro", vector_short),
    vector_short = ifelse(`vector payload` == "lentiGuide-Puro + EF1alpha_dCas9", "dCas9+Puro", vector_short),
    vector_short = factor(vector_short, levels = c('Thy1', 'Puro', 'dCas9+Puro')),
    mouse_model_short = ifelse(grepl('NSG', MODEL), 'NSG', 'Syngeneic'),
    group = paste(mouse_model_short, sample_type, vector_short)
  ) 

x %>% 
  ggplot(aes(vector_short, `% shown in figure`,
             fill = vector_short)) +
  geom_quasirandom(shape = 21, alpha = 0.8, color = 'black') +
  # scale_color_manual(values = palette_vector_line) +
  scale_fill_manual(values = palette_vector) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  # scale_y_continuous(
  ...
  #   # breaks = c(0, 0.5, 0.10, 0.25, 0.50, 0.75, 1.00),
  #   labels = scales::percent_format(accuracy = 1)
  #   ) +
  facet_grid2(sample_type ~ mouse_model_short,
              # scales = 'free_y',
...
```{r complexity-dynamics-padj, eval = FALSE}
library(rstatix)
x %>% 
  filter(sample_type == 'Primary tumor' & mouse_model_short == 'NSG') %>% 
  pairwise_wilcox_test(
    pct_shown_in_figure ~ group,
    comparisons = NULL,
    ref.group = NULL,
    p.adjust.method = "holm",
    detailed = FALSE
...
wt_res <- foreach(i = levels(x$sample_type), .combine = rbind) %do% {
  foreach(j = unique(x$mouse_model_short), .combine = rbind) %do% {
    x %>% 
      filter(sample_type == i & mouse_model_short == j) %>% 
      pairwise_wilcox_test(
        pct_shown_in_figure ~ vector_short,
        comparisons = NULL,
        ref.group = NULL,
        p.adjust.method = "holm",
        detailed = FALSE
      ...
      select(sample_type, mouse_model_short, everything())
